...
function statusMessage(status: string | undefined) {
  ...
  if (status === "email_changed") {
    return "E-mail byl změněn a potvrzovací odkaz byl odeslán na novou adresu.";
  }
  return null;
}
...
function errorMessage(error: string | undefined, waitSeconds?: number) {
  ...
  if (error === "email_exists") {
    return "Tento e-mail už je v systému použitý.";
  }
    ...
    <PortalCard title={cardTitle(phase)}>
      ...
      <div className="mt-6 space-y-4">
        ...
        <div className="flex flex-wrap gap-3">
          <form action={portalResendEmailConfirmationAction}>
        ...
        {canChangeEmail && showChangeEmail ? (
          <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-4">
            <form action={portalChangeUnverifiedEmailAction} className="space-y-3">
              ...
              <div>
                ...
                <input
                  ...
                  readOnly
                  value={email}
                  className="w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-400"
                />
              </div>
              <div>
                <label className="mb-1 block text-xs text-slate-500">
              ...
              >
                Uložit nový e-mail a poslat potvrzení
              </button>
            </form>
          </div>
        ) : null}
