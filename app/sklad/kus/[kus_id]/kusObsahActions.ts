...
function redirectWithObsahMessage(
  parentKusId: string,
...
export async function insertKusIntoCaseAction(formData: FormData) {
  ...
  const childKusId = String(formData.get("child_kus_id") ?? "").trim();
  ...
  try {
    ...
    redirectWithObsahMessage(parentKusId, { ok: "inserted" });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Vložení kusu do case se nezdařilo.";
    redirectWithObsahMessage(parentKusId, { error: message });
  }
}
...
export async function removeKusFromCaseAction(formData: FormData) {
  ...
  try {
    ...
    redirectWithObsahMessage(parentKusId, { ok: "removed" });
  } catch (error) {
